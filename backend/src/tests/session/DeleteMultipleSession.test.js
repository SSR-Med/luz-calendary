...
describe("DELETE /session/patient/id/:id", ()=>{

    test("Delete multiple session", async()=>{
        return request(app)
            .delete('/session/patient/id/1?start_date=2024-04-01&finish_date=2024-04-08')
    ...
    test("Get 404 if patient not found", async()=>{
        return request(app)
            .delete('/session/patient/id/20?start_date=2024-04-01&finish_date=2024-04-08')
            .auth(token, {type: 'bearer'})
            .expect(404)
    })
    ...
    test("Get 400 if invalid date", async()=>{
        return request(app)
            .delete('/session/patient/id/1?start_date=a&finish_date=2024-04-09')
            .auth(token, {type: 'bearer'})
            .expect(400)
    })
    ...
    test("Get 400 if start date > finish date", async()=>{
        return request(app)
            .delete('/session/patient/id/1?start_date=2024-04-09&finish_date=2024-04-01')
            .auth(token, {type: 'bearer'})
            .expect(400)
    })
